...
const getTopAnime = async (req, res) => {
  try {
    const url = await axios.get(`https://otakudesu.su/anime?order=popular`);
    const $ = await cheerio.load(url.data);
    const popular = [];
...
const getAnimeDetails = async (req, res) => {
  try {
    ...
    let text = $(
      "div.infox > div.ninfo > div > div.spe > span:nth-child(5)"
    ).text();

    let released =
      $("div.infox > div.ninfo > div > div.spe > span.split")
        .text()
        .match(/Released\s*:\s*(\d{4})/i)[1] || "";

    let type = $("div.infox > div.ninfo > div > div.spe > span:nth-child(6)")
      .text()
      .replace(/Type:/i, "");
    let episodeURL = [];

    $(" div.num > li > a:last-child").each((i, el) => {
      episodeURL.push(
        $(el)
          .attr("href")
          .replace(/^https?:\/\/otakudesu\.su\d+\//i, "")
      ...
      data: [
        {
          ...
          synopsis: $(
            "div.infox > div.ninfo > div > div.entry-content > p"
          ).text(),
          image: $("div.thumbook > div.thumb > img").attr("src"),
          status: $(
            " div.infox > div.ninfo > div > div.spe > span:nth-child(1)"
          )
            .text()
            .replace(/Status:/i, ""),
          studio: $(
            "div.infox > div.ninfo > div > div.spe > span:nth-child(2) > a"
          ).text(),
          released: released,
          genres: $("div.infox > div.ninfo > div > div.genxed > a")
            .text()
            .split(/(?=[A-Z])/),
          season: $(
            "div.infox > div.ninfo > div > div.spe > span:nth-child(5) > a"
          ).text(),
          episodeList: episodeURL,
        },
      ],
    });
  } catch (err) {
...
const getListAnime = async (req, res) => {
  try {
    const url = await axios.get(
      `https://otakudesu.su/anime?order=${req.query.order_by}`
    );
    const $ = await cheerio.load(url.data);
    const popular = [];
...
const getWatchAnime = async (req, res) => {
  try {
    const url = await axios.get(`https://otakudesu.su/${req.params.animeId}`);
    const $ = await cheerio.load(url.data);
    const popular = [];
    ...
    res.status(200).json({
      status: "success",
      data: {
        episodeURL: $("#pembed > iframe").attr("src"),
      },
    });
  } catch (err) {
    console.log(err);
